...
YouTube channel logs one warning and then goes quiet (see
[Limitations](#limitations)) — and it starts chiming automatically once it
goes live. (If you only wanted the
...
unparseable page and reports both the same way: one warning at the start of
the outage, then silence until a poll succeeds. Because a new broadcast has a
different stream id, it re-baselines silently instead of chiming against the
old count.
